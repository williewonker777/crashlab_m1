...
챕터 표지는 44페이지, 본문은 45~62페이지의 18장, 실습은 63~64페이지의 2장이다. 권장 진행은 설명 25~30분, 두 시연과 토의 10~15분이다. 물체에 접근하고 잡고 옮기는 작업을 중심으로 순기구학(FK, 정기구학이라고도 함)·역기구학(IK)을 연결한다.

## 구성

| 페이지 | 핵심 내용 |
| --- | --- |
| 45~48 | 매니퓰레이션의 목표, M1의 어깨 3·팔꿈치 1·손목 3축, 관절 공간과 작업 공간, TCP |
| 49~51 | 사진으로 보는 FK, 변환의 연결, 7개 관절각을 직접 조절하는 시연 |
| 52~54 | 목표 pose의 IK, 같은 손끝과 다른 팔 자세, Jacobian을 이용한 수치 IK |
| 55~57 | 작업공간·방향 제약, 특이점, 관절 한계·충돌·경로 연속성 |
| 58~60 | 접근·파지·들어 올리기, 자유 이동과 직선 접근, 궤적·관절 제어·피드백 |
| 61~62 | 카메라의 물체 pose에서 파지 목표까지의 변환, 시연을 이용한 확인 문제 |
| 63~64 | 실습 — Pinocchio로 FK를 만들어 로봇 TF와 대조, 자코비안 IK를 만들어 왕복 검증 후 팔에 송출 |
...
51페이지에서는 q₁~q₇, 위치, quaternion을 연결해서 본다. 입력은 도 단위로 표시하고 내부에서 rad로 바꾼다. 입체·정면·측면 보기와 세 가지 관절 자세 프리셋을 제공한다. 손목 회전도 TCP 오프셋과 회전축에 따라 위치와 방향에 영향을 줄 수 있다.

53페이지에서는 기준 관절각 `[-30, 25, 10, -70, 15, 25, 0]°`의 전체 TCP pose를 목표로 고정한다. q₃를 −40°~60°로 정한 뒤 나머지 6개 관절을 수치 IK로 계산한다. 이 범위의 예제에서는 팔꿈치 위치가 달라져도 TCP 위치와 방향을 유지한다. 이는 모든 목표나 모든 관절 제한에서 가능한 동작이라는 뜻은 아니다.
...
51·53페이지의 시연을 학생이 Pinocchio로 다시 만드는 것이 목표다. 참고 구현은
`~/crashlab_fkik`에 있다.
...
| 51페이지 슬라이더 `assets/js/arm-model.js` | 0.100 m | −0.615 |
