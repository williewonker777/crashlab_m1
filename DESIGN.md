...
# v12 — 강의 ③ 구현 사례·시행착오 12장
2026-09-14. 사용자 직접 지시. 강의 ③의 토픽 흐름만 보고 직접 만든 VSLAM
스택(`~/crashlab_vslam`)의 시행착오를 강의 안으로 들여온다.

- 이론(47) 뒤, **실습 앞**에 12장(48~59)을 넣는다. 이론이 개념을 세우고, 이 12장이 그
  개념을 실제로 구현했을 때 무엇이 깨졌는지 보여 준 다음, 실습에서 직접 재 보는 순서다.
  실습은 60~65로 밀린다.
- 범위는 **VSLAM·SLAM으로 한정한다.** 같은 프로젝트의 내비게이션(코스트맵·A*·순수 추종·
  주행 결과) 슬라이드는 넣지 않는다 — 강의 ③의 내비게이션 이론은 33~45에서 이미 다루고,
  구현 사례까지 양쪽을 다루면 한 챕터가 두 주제를 끌고 간다.
- 흐름: 질문 → 만든 것 → 실패 ①뎁스 컷 → 결과 → 실패 ②그래프 엣지 → 결과 →
  실패 ③레이캐스팅 → 실패 ④방향 의존 → 실패 ⑤드리프트 처방 다섯 → 진짜 원인 →
  측정 방법 → 정리.
- **모든 숫자는 실측값이다.** 예시를 위해 지어낸 값을 쓰지 않는다. 출처는
  `~/crashlab_vslam/README.md`이며, 결과 그림 2장은 `tools/plot_*.py`가 만든 원본이다
  (`assets/img/impl-01..02`).
- 실패 슬라이드는 증상이 아니라 **무엇을 어떻게 쟀는지**를 싣는다. 고친 방법만 남기면
  "그렇게 하면 된다"가 되고, 잰 값을 남겨야 왜 그렇게 됐는지가 남는다.
- 잘못된 처방 다섯 가지(s56)를 지우지 않고 표로 남긴다. 그럴듯한 처방이 측정에서
  어떻게 깨지는지가 이 장의 요지다.
- 실습보다 앞이므로 **실습 번호를 앞질러 참조하지 않는다.** 뎁스 오차식은 실습 6이 아니라
  슬라이드 11을 가리킨다.
- 새 컴포넌트 둘: `.metric-table`(다열 실측 표 + `.metric-note`), `.result-figure`
  (원본 결과 이미지 + 캡션). 기존 `concept-table`은 2열 전용이라 실측 표를 못 받는다.
- 구성은 §B-2를 지켜 좌우를 번갈아 쓴다: 50 split / 52 split-reverse / 54 split /
  55 split / 58 split-reverse.
- 강의 ③은 65장, 전체 강의는 155장이다.
